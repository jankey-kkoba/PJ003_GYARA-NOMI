...
2. **Playwrightを起動して実際の動作を確認**
   - 自動テストだけでなく、実際のブラウザで目視確認を行う
   - **必ずスマホサイズ（375x667等）でブラウザをリサイズして確認すること**（モバイルファースト）
   - レスポンシブ表示、アニメーション、UXなどテストで確認しにくい部分を重点的に確認
   ```
   # Playwrightでスマホサイズにリサイズ
...
5. **動作確認完了後のクリーンアップ（必須）**
   - Playwrightブラウザを閉じる（`mcp__playwright__browser_close`）
   - **開発サーバーを停止する**（`KillShell`でバックグラウンドプロセスを終了）
   - リソースを解放してから次の作業に進む
