...
const { CastListTemplate } =
	await import('@/features/cast/components/organisms/CastListTemplate')
...
describe('CastListTemplate', () => {
	...
	describe('エラー状態', () => {
		it('API エラー時はエラーメッセージを表示する', async () => {
			...
			await expect
				.element(page.getByText(/エラーが発生しました/))
				.toBeInTheDocument()
		})
	...
	describe('空の状態', () => {
		it('キャストが0件の場合は空メッセージを表示する', async () => {
			...
			await expect
				.element(page.getByText('キャストが見つかりませんでした'))
				.toBeInTheDocument()
		})
	})
	...
	describe('キャスト一覧の表示', () => {
		it('キャストカードを正しく表示する', async () => {
			...
			await expect
				.element(page.getByText('テストキャスト...').first())
				.toBeInTheDocument()
		...
		it('サムネイル画像がある場合、画像を表示する', async () => {
				...
				casts: [
					{
						...
						areaName: '渋谷',
						thumbnailUrl:
							'https://example.com/storage/v1/object/public/cast-profile-photos/test.jpg',
					},
				],
				total: 1,
				page: 1,
			...
			// 画像が表示されることを確認
			await expect
				.element(page.getByAltText('テストキャストのプロフィール写真'))
				.toBeInTheDocument()
			// 「画像未設定」プレースホルダーは表示されない
			await expect.element(page.getByText('画像未設定')).not.toBeInTheDocument()
		})

		it('サムネイル画像がない場合、プレースホルダーを表示する', async () => {
			const mockData: CastListResponse = {
				casts: [
					{
						id: 'cast-1',
			...
			// 「画像未設定」プレースホルダーが表示されることを確認
			await expect.element(page.getByText('画像未設定')).toBeInTheDocument()
		})
	})
	...
	describe('ページネーション', () => {
		...
		it('ページ変更時に新しいデータを取得する', async () => {
			...
			const page2Button = paginationNav.getByRole('link', {
				name: '2',
				exact: true,
			})
