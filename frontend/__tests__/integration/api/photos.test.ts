...
vi.mock(
	'@/features/cast-profile-photo/services/photoService',
	async (importOriginal) => {
		const actual =
			await importOriginal<
				typeof import('@/features/cast-profile-photo/services/photoService')
			>()
		return {
			photoService: {
				...actual.photoService,
...
const { storageService } =
	await import('@/features/cast-profile-photo/services/storageService')
const mockStorageService = vi.mocked(storageService)

// モック化された photoService を取得
const { photoService } =
	await import('@/features/cast-profile-photo/services/photoService')
