...
// モック後にインポート
const { useRegisterUser } = await import('@/features/user/hooks/useRegisterUser')
...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <div data-testid="data">{mutation.data ? JSON.stringify(mutation.data) : 'none'}</div>
      <button
        onClick={() =>
          mutation.mutate({
            name: 'テストユーザー',
...
describe('useRegisterUser', () => {
  ...
  describe('初期状態', () => {
    it('初期状態では idle 状態', async () => {
      ...
      await expect.element(page.getByTestId('isSuccess')).toHaveTextContent('false')
      await expect.element(page.getByTestId('isError')).toHaveTextContent('false')
    })
  })
  ...
  describe('成功ケース', () => {
    ...
    it('成功時にレスポンスデータが data に設定される', async () => {
      ...
      await expect.element(page.getByTestId('isSuccess')).toHaveTextContent('true')
      await expect.element(page.getByTestId('data')).toHaveTextContent(JSON.stringify(mockResponse))
    })
  })
  ...
  describe('エラーケース', () => {
    it('APIがエラーを返した場合、エラー状態になる', async () => {
      ...
      await expect.element(page.getByTestId('error')).toHaveTextContent('認証が必要です')
    })

    it('バリデーションエラーの場合、エラーメッセージが設定される', async () => {
      mockPost.mockResolvedValue({
        ok: false,
        json: () => Promise.resolve({ error: '名前は2文字以上で入力してください' }),
      })
    ...
    it('エラーレスポンスに error フィールドがない場合、デフォルトメッセージを使用', async () => {
      ...
      await expect.element(page.getByTestId('isError')).toHaveTextContent('true')
      await expect.element(page.getByTestId('error')).toHaveTextContent('登録に失敗しました')
    })
