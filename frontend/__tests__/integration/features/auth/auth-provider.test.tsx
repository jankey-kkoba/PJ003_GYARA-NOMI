...
describe('AuthProvider', () => {
	...
	describe('レンダリング', () => {
		it('子コンポーネントをレンダリングする', async () => {
			...
			await expect
				.element(page.getByTestId('test-child'))
				.toHaveTextContent('Test Child')
		})
		...
		it('複数の子コンポーネントをレンダリングする', async () => {
			...
			await expect
				.element(page.getByTestId('child-1'))
				.toHaveTextContent('Child 1')
			await expect
				.element(page.getByTestId('child-2'))
				.toHaveTextContent('Child 2')
			await expect
				.element(page.getByTestId('child-3'))
				.toHaveTextContent('Child 3')
		})
	...
	describe('ネストされた構造', () => {
		...
		it('複雑なコンポーネント構造で認証状態が伝わる', async () => {
			...
			await expect
				.element(page.getByTestId('user-email'))
				.toHaveTextContent('nested@example.com')
		})
	})
	...
	describe('children のタイプ', () => {
		...
		it('Fragment を children として渡せる', async () => {
			...
			await expect
				.element(page.getByTestId('fragment-1'))
				.toHaveTextContent('Fragment 1')
			await expect
				.element(page.getByTestId('fragment-2'))
				.toHaveTextContent('Fragment 2')
		})
	})
})
