...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <button onClick={() => auth.lineLogin('guest')} data-testid="login-guest-btn">
        Login Guest
      </button>
      <button onClick={() => auth.lineLogin('cast')} data-testid="login-cast-btn">
        Login Cast
      </button>
...
describe('useAuth', () => {
  ...
  describe('セッション状態', () => {
    it('認証済みの場合 isAuthenticated が true を返す', async () => {
      ...
      await expect.element(page.getByTestId('isAuthenticated')).toHaveTextContent('true')
    })
    ...
    it('未認証の場合 isAuthenticated が false を返す', async () => {
      ...
      await expect.element(page.getByTestId('isAuthenticated')).toHaveTextContent('false')
    })
    ...
    it('セッションが存在する場合 user オブジェクトを返す', async () => {
      ...
      await expect.element(page.getByTestId('userId')).toHaveTextContent('user-123')
      await expect.element(page.getByTestId('userEmail')).toHaveTextContent('test@example.com')
    })
    ...
    it('セッションが存在しない場合 user は none を表示する', async () => {
      ...
      await expect.element(page.getByTestId('userEmail')).toHaveTextContent('none')
    })
  })
