...
function TestComponent({
  ...
  const { mutate, isPending, isSuccess, isError, data, error } = useCreateSoloMatching()
...
describe('useCreateSoloMatching', () => {
  ...
  describe('エラーハンドリング', () => {
    it('APIエラー時にトーストでエラーメッセージを表示', async () => {
      ...
      expect(mockShowToast).toHaveBeenCalledWith('キャストが見つかりません', 'error')
    })
  ...
  describe('ローディング状態', () => {
    it('送信中はisPendingがtrueになる', async () => {
      ...
      await vi.waitFor(async () => {
        await expect.element(page.getByTestId('isPending')).toHaveTextContent('true')
      })
