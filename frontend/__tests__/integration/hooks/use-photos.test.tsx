...
// モック後にインポート
const { usePhotos } =
	await import('@/features/cast-profile-photo/hooks/usePhotos')
const { useUploadPhoto } =
	await import('@/features/cast-profile-photo/hooks/useUploadPhoto')
const { useDeletePhoto } =
	await import('@/features/cast-profile-photo/hooks/useDeletePhoto')
...
describe('写真管理hooks', () => {
	...
	describe('usePhotos', () => {
		it('写真一覧を正常に取得できる', async () => {
			...
			await expect
				.element(page.getByTestId('photo-0'))
				.toHaveTextContent('photo-1')
			await expect
				.element(page.getByTestId('photo-1'))
				.toHaveTextContent('photo-2')
		})
		...
		it('APIエラー時はエラーメッセージを返す', async () => {
			...
			await expect
				...
				.toHaveTextContent('写真の取得に失敗しました')
		})
	})
	...
	describe('useUploadPhoto', () => {
		...
		it('アップロード失敗時はエラーメッセージを返す', async () => {
			...
			await expect
				.element(page.getByTestId('error'))
				.toHaveTextContent('ファイルサイズが大きすぎます')
		})
	})
