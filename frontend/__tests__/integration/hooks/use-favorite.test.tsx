...
// モック後にインポート
const { useFavoriteStatus, useAddFavorite, useRemoveFavorite } =
	await import('@/features/favorite/hooks/useFavorite')
...
function TestMutationComponent({ castId }: { castId: string }) {
	...
	return (
		<div>
			...
			<button data-testid="add-btn" onClick={() => addFavorite.mutate(castId)}>
				追加
			</button>
			<button
...
describe('useFavoriteStatus', () => {
	...
	it('お気に入り状態を取得できる（登録済み）', async () => {
		...
		await expect
			.element(page.getByTestId('isFavorite'))
...
describe('useAddFavorite', () => {
	...
	it('お気に入りを追加できる', async () => {
		...
		await expect
			.element(page.getByTestId('add-pending'))
			.toHaveTextContent('false')
...
describe('useRemoveFavorite', () => {
	...
	it('お気に入りを削除できる', async () => {
		...
		await expect
			.element(page.getByTestId('remove-pending'))
			.toHaveTextContent('false')
			...
			return (
				<div>
					<div data-testid="isFavorite">
						{String(status.data ?? 'undefined')}
					</div>
					<button
						data-testid="add-btn"
						onClick={() => addFavorite.mutate('cast-123')}
					>
						追加
					</button>
				</div>
